...
from keras.layers import Input, Conv1D, LeakyReLU, ReLU, Softmax, ELU, MaxPool1D, CuDNNLSTM, Bidirectional, TimeDistributed, Dense, Reshape
...
def temporal_autoencoder(input_dim, timesteps, n_filters=50, kernel_size=10, strides=1, pool_size=10, n_units=[50, 1]):
    ...
    # Encoder
    """
    encoded = Conv1D(n_filters, kernel_size, strides=strides, padding='same', activation='linear')(x) # why linear activation? I need to see other studies
    encoded = LeakyReLU()(encoded) # why leaky relu? I need to see other studies
    encoded = MaxPool1D(pool_size)(encoded)
    
    encoded = Bidirectional(CuDNNLSTM(n_units[0], return_sequences=True), merge_mode='sum')(encoded) # why biLSTM? I need to see other studiess
    if n_units[1] > 0:
        encoded = LeakyReLU()(encoded)
        encoded = Bidirectional(CuDNNLSTM(n_units[1], return_sequences=True), merge_mode='sum')(encoded)
    if n_units[2] > 0:
        encoded = LeakyReLU()(encoded)
        encoded = Bidirectional(CuDNNLSTM(n_units[2], return_sequences=True), merge_mode='sum')(encoded)
    encoded = LeakyReLU(name='latent')(encoded) # why leaky relu? I need to see other studies
    ...
    """
    if n_units[2] < 1:
        input_size_decoder = n_units[1]
    if n_units[1] < 1:
        input_size_decoder = n_units[0]    
    decoded = Reshape((-1, 1, input_size_decoder), name='reshape')(encoded)
